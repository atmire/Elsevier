...
importClass(Packages.com.atmire.submit.step.SourceChoiceStep);
    ...
    do {
        ...
        if ((NEXT_BUTTON || !stepHasUI(stepConfig)) && (response_flag == AbstractProcessingStep.STATUS_COMPLETE)) {
            ...
            if (state.progressIterator >= totalSteps) {
                ...
                else {   //since in Workflow just break out of loop to return to Workflow process
                    break;
                }
            }
            else {
...
function doNextPage(collectionHandle, workspaceID, stepConfig, stepAndPage, response_flag) {
    ...
    if (stepHasUI(stepConfig)) {
        ...
        sendPageAndWait("handle/" + collectionHandle + "/submit/continue", {
            "id": workspaceID,
            "step": String(stepAndPage),
            "transformer": stepConfig.getXMLUIClassName(),
            "error": String(response_flag),
            "error_fields": getErrorFields()
        });
    }
...
function submitStepSaveOrRemove(collectionHandle, workspaceID, step, page) {
    ...
    sendPageAndWait("handle/" + collectionHandle + "/submit/saveOrRemoveStep", {
        "id": workspaceID,
        "step": String(step),
        "page": String(page)
    });
