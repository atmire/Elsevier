package com.atmire.dspace.submit.step;
 ...
 * @version $Revision$
 */
public class ElsevierUploadStep extends AbstractProcessingStep {
    ...
    public int doProcessing(Context context, HttpServletRequest request,
        ...
        if (bitstreamId != null) {
            ...
            if (fileAccess == null) {
                ...
                Bitstream b = Bitstream.find(context, Integer.parseInt(bitstreamId));
                MetadataUtils.clearMetadata(b, "workflow.fileaccess");
                MetadataUtils.addMetadata(b, "workflow.fileaccess", fileAccess);

                if(fileAccess.equals("embargo")) {
                    DCDate embargoDate = FileAccess.getEmbargoDate(request);

                    if (embargoDate.toDate() != null) {
                        MetadataUtils.clearMetadata(b, "workflow.fileaccess.date");
                        MetadataUtils.addMetadata(b, "workflow.fileaccess.date", embargoDate.toString());
                    }

                    if(FileAccess.fileAccessIdentical(context,b)){
                        MetadataUtils.addMetadata(b, "workflow.fileaccess.overruled", "false");
                    }
                    else {
                        MetadataUtils.addMetadata(b, "workflow.fileaccess.overruled", "true");
                    }
                }
        ...
        if (request.getParameter("primary_bitstream_id") != null) {
            ...
            if (bundles.length > 0) {
                int primary_bitstream_id = Integer.valueOf(request.getParameter("primary_bitstream_id"));
                bundles[0].setPrimaryBitstreamID(primary_bitstream_id);
                bundles[0].update();
            }
        }
